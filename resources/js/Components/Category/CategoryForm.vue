...
<template>
    <div :class="{'translate-x-0': props.isOpen, 'translate-x-full': !props.isOpen}" class="fixed scrollbar-none overflow-y-auto right-0 top-0 bg-white transition-all duration-300 bottom-0 min-w-[350px] max-w-[350px] z-50 shadow-xl p-3">
        <div class="flex justify-between w-full p-2">
            <div class="font-bold text-slate-600">{{ props.addingCategory ? 'ADD NEW CATEGORY' : 'UPDATE CATEGORY' }}</div>
        ...
        <form class="grid grid-cols-1 gap-y-4 pt-2 w-max-full" @submit.prevent="emit('submit')">
            <!-- Category -->
            <div class="w-full">
                <p class="text-sm font-extrabold text-slate-800 ps-1 pb-1">Category:</p>
                <input type="text" class="w-full" v-model="props.form.category" placeholder="Sub Category" />
                <p class="text-red-500 text-sm" v-if="props.form.errors.category">{{ props.form.errors.category }}</p>
            </div>
            <!-- Submit Button -->
